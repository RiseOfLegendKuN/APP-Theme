...
function nav(path) {
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <a class="nav-link" href="${UI.website_link}" target="_blank">${UI.nav_link_website}</a>
...
function append_files_to_list(path, files) {
	...
	for (i in files) {
      ...
      if ("|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${ext}|`) >= 0) {
        html += video_icon
      } else if ("|html|php|css|go|java|js|json|txt|sh|ass|ssa|srt|".indexOf(`|${ext}|`) >= 0) {
        html += code_icon
      } else if ("|zip|rar|tar|.7z|.gz|".indexOf(`|${ext}|`) >= 0) {
        html += zip_icon
      } else if ("|bmp|jpg|jpeg|png|gif|".indexOf(`|${ext}|`) >= 0) {
...
// Document display subtitle files (ass, ssa, srt)
function file_subtitle(name, encoded_name, size, url, file_id, cookie_folder_id) {
